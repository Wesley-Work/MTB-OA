  ...
  <div class="view">
    ...
    <div class="taskList">
      <div
        ...
        :key="item"
        class="taskList--item"
        :style="{ order: getTagPriority(item.type) }"
      >
        <div class="task-title">
          <div class="task-tag">
            ...
            <t-tag
              ...
              :color="getTypeOrStatus(taskStatus, item.status)?.color"
              >{{ getTypeOrStatus(taskStatus, item.status)?.label }}</t-tag
            >
          </div>
          {{ item.id }}# {{ item.name }}
        </div>
        <div class="taskItem">
          ...
          <div class="taskItem--workTime">
            工作时间：{{ taskTimeConvert((item.work_time as string)?.split(','))?.join(' 至 ') }}
          </div>
