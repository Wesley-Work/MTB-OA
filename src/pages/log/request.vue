...
const InitTableData = (date = getToday(), push = false) => {
  ...
  useRequest({
    ...
    success: (res) => {
      ...
      if (push) {
        RES.data[0].data.forEach((element, index) => {
...
const InitDatePickerData = () => {
  ...
  useRequest({
    ...
    success: (res) => {
      const RES = JSON.parse(res);
      DateSelectData.value = RES.data.map((item) => item.date);
      table_Loading.value = false;
    },
    error: (err) => {
      table_Loading.value = false;
...
const InitDateDateSelectData = (e) => {
  const date = getToday(e);
  return !DateSelectData.value.includes(date);
};
...
const TableSortData = () => {
  ...
  if (table_Sort) {
    table_Data.value = data.sort((a, b) =>
      table_Sort.descending ? b[table_Sort.sortBy] - a[table_Sort.sortBy] : a[table_Sort.sortBy] - b[table_Sort.sortBy],
    );
  } else {
