<template>
    ...
    <t-menu :theme="theme ? 'dark' : 'light'" :value="SideMenu.value" height="550px" class="sidemenu"
        ...
        <t-menu-group title="晚修/自修管理">
            <t-menu-item value="WXGL">
                <template #icon>
        ...
        <!---->
        <!-- <t-menu-group title="监控管理">
            <t-menu-item value="CCTV">
                <template #icon>
                    <t-icon name="camera" />
                </template>
                监控预览
            </t-menu-item>
            <t-menu-item value="CCTV2">
                <template #icon>
                    <t-icon name="play-rectangle" />
                </template>
                监控回放
            </t-menu-item>
        </t-menu-group> -->
    ...
    <div style="padding: 24px" class="MainContent" :class="{ 'SideMenuShow-MainContent': SideMenu.show }"
        ...
        <div style="padding-bottom: 24px; user-select: none" v-if="MainContent.breadcrumb.show">
        ...
        <section class="loading-change-components-animation" :class="{
            ...
            <div id="copyright">
                <div>
                    ...
                    &
                    <a href="https://ddos-ling.top/" we-a-tag target="_blank" @click.prevent="NotClick">DDoS_LING</a>
                    | Designed By
    ...
    <!-- guide引导 -->
    <t-guide v-model="guide.current" :steps="guide.steps" @finish="hide_guide" @skip="hide_guide"
        onChange="hangleGuideChange" />
    <t-dialog v-model:visible="guide.confirmDialog" header="提示" body="将开启引导图层，将带您了解本系统各项功能，预计需要5-8分钟。" attach="body"
        :confirm-on-enter="false" :on-cancel="hide_guideDialog" :on-esc-keydown="hide_guideDialog"
        :on-close-btn-click="hide_guideDialog" :on-overlay-click="hide_guideDialog" :on-close="hide_guideDialog"
        :on-confirm="show_guide" />
</template>
...
import Component from "@/components/function/component/index.js";
import { PoweroffIcon, UserIcon, ChatBubbleHelpIcon } from "tdesign-icons-vue-next";
import { NotifyPlugin } from "tdesign-vue-next";
import GuideTemplate from "@/components/pages/GuidePopup.vue";
import { HTTPRequest, VerifyToken, GetAPIUrl, GetSSOUrl } from '@/components/function/hooks.js'

let guide_title = "新手引导";
var Version
...
export default {
    ...
    data() {
        return {
            ...
            guide: {
                ...
                steps: [
                    {
                        element: ".guide_toggletheme",
                        title: guide_title,
                        content: GuideTemplate,
                        body: "123df",
                        placement: "top",
                    },
                    {
                    ...
                    {
                        element: ".guide_accountdropdown",
                        title: guide_title,
                        content: GuideTemplate,
                        placement: "top",
                    },
                    {
                        element: ".guide_sidemenus",
                        title: guide_title,
                        content: GuideTemplate,
                        placement: "top-right",
                    },
                    {
                        element: ".guide_helplend",
                        title: guide_title,
                        content: GuideTemplate,
                        placement: "left",
                    },
                    {
                        element: ".guide_guestlend",
                        title: guide_title,
                        content: GuideTemplate,
                        placement: "left",
                    },
                    {
                        element: ".guide_lendinput",
                        title: guide_title,
                        content: GuideTemplate,
                        placement: "left",
                    },
    ...
    mounted() {
        ...
        var param_PureMode = this.getUrlParam("puremode");
        var param_LargeScreen = this.getUrlParam("largescreen");
        var __DEBUG_DONTCHECKLOGINSTATUS = this.getUrlParam("__DEBUG_DONTCHECKLOGINSTATUS");// DEBUG: 不检测TOKEN
        if (param_PureMode == "true" || param_LargeScreen == "true") {
            console.log("纯净模式（大屏模式）开启，隐藏全部菜单，显示数据页面。");
            this.$data.TitleMenu.show = false;
            this.$data.MainContent.breadcrumb.show = false;
            this.SideMenuValueChange("Dashboard", true, false);
        }
    ...
    methods: {
        ...
        //遇到问题
        HasProblem() {
            this.$data.guide.confirmDialog = true;
        },
         ...
         */
        hide_guideDialog() {
            this.$data.guide.confirmDialog = false;
        },
         ...
         */
        show_guide() {
            this.hide_guideDialog();
            setTimeout(() => {
                this.$data.guide.current = 0;
            }, 250);
        },
        ...
        hangleGuideChange(e) {
            if (e == 4) {
                this.SideMenuValueChange("lend", true, false);
            }
        },
